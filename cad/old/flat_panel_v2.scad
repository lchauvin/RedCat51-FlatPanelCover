...
// Arm bracket on main_body: rib at body-bottom (z=0), bracket post at x_break≈55 mm.
// Linkage arm L-shape bolts to post via slot; hub mounts on D-shaft at (shaft_cx, 0, 25).
...
// ═══════════════════════════════════════════════════════════════
//  PART 2 — DIFFUSER RING   (print × 2, identical)
//
//  Holds a 56×56 mm square of 3 mm frosted acrylic (score-and-snap).
//  Both rings are the same part — no arm variant needed.
//  Pivot arm is now on main_body as a Z-shaped crank.
//
//  with_skirt = false (default) — use foam gasket (Option A).
//  with_skirt = true  — Option B: adds a continuous light-sealing
//                       skirt that slides over adapter_od.
//
//  Print both rings: diffuser_ring()  (or with_skirt=ENABLE_SKIRT for bottom)
// ═══════════════════════════════════════════════════════════════
...
module diffuser_ring(with_skirt = false) {
    difference() {
        union() {
            ...
            // Option B: continuous light-sealing skirt below ring
            if (with_skirt) {
                difference() {
        ...
        // Circular light aperture — from ring bottom up to shelf
        translate([0, 0, -1])
...
// ═══════════════════════════════════════════════════════════════
//  PART 4 — MAIN BODY (LED TRAY + ARM BRACKET)
//
...
//
//  Arm mounting bracket on +X side:
//    • Thin rib exits body wall at body-bottom (z=0 local) — safe
//      below the adapter sweep zone.
//    • Bracket post at x_break ≈ 55 mm (outside diffuser OD = 44 mm).
//    • Two M3 bolt holes through post in Y direction align with the
//      linkage_arm_L() slot for vertical adjustment.
//
//  Driver box mounting:
//    • Two M3 bosses on the sky-facing top face (z=body_h local).
//    • Bolts come from above through driver_enclosure() base.
//
...
module main_body() {
    ...
    // Bracket geometry
    x_break    = housing_od / 2 + (shaft_cx - housing_od / 2) / 2; // ≈ 55 mm
    bracket_h  = 8;   // post height (z_local = −1 to 7, covers arm slot range)
    bracket_w  = 12;  // post width in Y (arm thickness 5 mm + 3.5 mm clearance each side)
    bracket_d  = 8;   // post depth in X
    ...
    difference() {
        union() {
            ...
            // ── Rib: connects body wall to bracket post at body-bottom (z=0 → arm_t)
            translate([housing_od / 2, -bracket_w / 2, 0])
                cube([x_break - housing_od / 2, bracket_w, arm_t]);

            // ── Bracket post: arm bolts here through its adjustable slot
            translate([x_break - bracket_d / 2, -bracket_w / 2, -1])
                cube([bracket_d, bracket_w, bracket_h]);

            // ── Driver box mounting bosses on sky-facing top face
            for (yy = [-12, 12])
                translate([0, yy, body_h])
                    cylinder(d = m3_head_d + 2, h = 3);
        }
        ...
        // ── 2× M3 bolt holes through bracket post (Y direction, for arm slot)
        for (zz = [1, 5])
            translate([x_break, -bracket_w / 2 - 1, zz])
                rotate([-90, 0, 0])
                    cylinder(d = m3_clear, h = bracket_w + 2);

        // ── Driver box bolt holes (from above into bosses)
        for (yy = [-12, 12])
            translate([0, yy, body_h - 1])
                cylinder(d = m3_clear, h = 5);
    }
}
...
//
//  Parametric box for the ULN2003 stepper driver breakout.
//  Update uln_pcb_w / uln_pcb_l / uln_comp_h once you have the real board.
//
//  Assembly: PCB drops in, lid snaps/screws down.
//  Mounts to sky-facing top face of main_body() via 2× M3 screws
//  (24 mm centre-to-centre in Y, matching bosses on body top face).
//
...
module driver_enclosure() {
    ...
    // Body mounting holes: 2× M3 through base, 24 mm apart in Y,
    // matching bosses on main_body() top face.
    mount_spacing = 24.0;
    ...
    difference() {
        ...
        // ── 2× M3 mounting holes through base — align with body top bosses
        for (yy = [ol / 2 - mount_spacing / 2, ol / 2 + mount_spacing / 2])
            translate([ow / 2, yy, -1])
                cylinder(d = m3_clear, h = uln_wall + 2);
    }
...
// ═══════════════════════════════════════════════════════════════
//  PART 9 — LINKAGE ARM  (separate printed part; use arm.scad to export)
//
//  L-shaped arm printed flat.  Hub mounts on D-shaft at (shaft_cx, 0, stepper_z).
//  Vertical segment bolts to main_body bracket through adjustable slot.
//
//  Assembly placement transform (puts arm in XZ plane, hub at shaft):
//    translate([shaft_cx, arm_thickness/2, stepper_z])
//        rotate([90, 0, 0])
//            rotate([0, 0, 180])
//                linkage_arm_L();
//
//  Parameters mirror arm.scad — edit both files together if geometry changes.
// ═══════════════════════════════════════════════════════════════

module linkage_arm_L() {
    arm_thickness        =  5;
    arm_pivot_d          = 14;
    arm_pivot_hole_d     =  5.4;   // D-shaft clearance
    arm_shaft_d          =  5.0;
    arm_shaft_flat       =  1.6;
    arm_horiz_len        = 11;
    arm_w                = 10;
    ...
    module arm_bar(p1, p2, w) {
        hull() {
            translate(p1) cylinder(h = arm_thickness, d = w);
            translate(p2) cylinder(h = arm_thickness, d = w);
        }
    }
    module arm_slot(p1, p2, w) {
        hull() {
            translate(p1) cylinder(h = arm_thickness + 1, d = w);
            translate(p2) cylinder(h = arm_thickness + 1, d = w);
        }
    }

    difference() {
        union() {
            cylinder(h = arm_thickness, d = arm_pivot_d);
        ...
        translate([0, 0, -0.5])
            linear_extrude(arm_thickness + 1)
                difference() {
                    circle(d = arm_pivot_hole_d);
                    translate([0, arm_shaft_flat + arm_shaft_d / 2])
                        square([arm_shaft_d + 1, arm_shaft_d], center = true);
                }
        ...
        arm_slot(
            [arm_horiz_len, -arm_vert_offset - arm_slot_margin],
            [arm_horiz_len, -arm_vert_offset - arm_slot_margin - arm_slot_len],
            arm_slot_w
        );
    }
}
            ...
            translate([-shaft_cx, 0, -stepper_z]) {
                color("white",   0.85)
                    translate([0, 0, z_diff1])
                        diffuser_ring(with_skirt = ENABLE_SKIRT);
                color("dimgray", 0.50)
                    translate([0, 0, z_gap])
                        air_gap_ring();
                color("white",   0.85)
                ...
                // ── Linkage arm — hub on D-shaft, slot bolts to body bracket
                color("lime", 0.85)
                    translate([shaft_cx, 5 / 2, stepper_z])
                        rotate([90, 0, 0])
                            rotate([0, 0, 180])
                                linkage_arm_L();
            }
...
// adapter_ring();                                    // ×1  black  (flat top face)
// motor_cover();                                     // ×1  black
// diffuser_ring(with_skirt = ENABLE_SKIRT);          // ×1  white  (bottom ring)
// diffuser_ring();                                   // ×1  white  (top ring — same part)
 air_gap_ring();                                    // ×1  black
// main_body();                                       // ×1  black  (bracket post + driver bosses)
// linkage_arm_L();                                   // ×1  black  (print flat; or use arm.scad)
// driver_enclosure();                                // ×1  black  (bolts to main_body top)
// driver_lid();                                      // ×1  black
